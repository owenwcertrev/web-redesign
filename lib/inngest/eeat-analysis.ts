...
export const comprehensiveBlogAnalysis = inngest.createFunction(
  ...
  async ({ event, step }: { event: BlogComprehensiveAnalysisEvent; step: any }) => {
    ...
    const aggregateNLP = await step.run('calculate-aggregate-nlp', async () => {
      ...
      const avgTone = nlpAnalyses.reduce((sum: number, a: any) => sum + a.nlp.toneScore, 0) / nlpAnalyses.length
      const avgExperience = nlpAnalyses.reduce((sum: number, a: any) => sum + a.nlp.experienceScore, 0) / nlpAnalyses.length
      const avgExpertise = nlpAnalyses.reduce((sum: number, a: any) => sum + a.nlp.expertiseDepthScore, 0) / nlpAnalyses.length
      const avgAI = nlpAnalyses.reduce((sum: number, a: any) => sum + a.nlp.aiContentScore, 0) / nlpAnalyses.length
